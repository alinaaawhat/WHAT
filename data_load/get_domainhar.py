...
from data_load.data_util.get_dataloader import load, load_client,load_client_fre
...
def get_acthar_client(args, dataset = 'pamap',target = 0, batch_size =64, remain_rate = 1, seed = 1, train_diff = 0, client_id = 1):
    # args = args_parse()
    args.step_per_epoch = 100000000000
    args.scaler_method = 'minmax'
    args.batch_size = batch_size
    ...
    print(args)
    data_load.utils.set_random_seed(args.seed)
    train_ori_loader, train_aug_loader, val_ori_loader, val_aug_loader, test_ori_loader, test_aug_loader = load_client(
        args, client_id)
    if train_diff:
        train_dataset = train_ori_loader.dataset
        valid_dataset = val_ori_loader.dataset
...
def get_acthar_client_fre(args, dataset = 'pamap',target = 0, batch_size =64, remain_rate = 1, seed = 1, train_diff = 0, client_id = 1, combine_train_aug = 0):
    ...
    print(args)
    data_load.utils.set_random_seed(args.seed)
    train_ori_loader, train_aug_loader, val_ori_loader, val_aug_loader, test_ori_loader, test_aug_loader = load_client_fre(
        args, client_id)
    if train_diff:
        train_dataset = train_ori_loader.dataset
        valid_dataset = val_ori_loader.dataset
