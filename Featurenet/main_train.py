...
parser.add_argument('--client_id', default=1, type=int,
                    help='current client id')
parser.add_argument('--num_clients', default=3, type=int,
                    help='total number of clients')
parser.add_argument('--enable_cross_client_generation', default=True, type=bool,
                    help='whether to enable cross-client style generation')
...
testuser['client_id'] = args.client_id
testuser['num_clients'] = args.num_clients
testuser['enable_cross_client_generation'] = args.enable_cross_client_generation

set_random_seed(testuser['seed'])
testuser['remain_data'] = args.remain_rate
print("Remain:",testuser['remain_data'])
print("Client ID:", testuser['client_id'])

# Modified name to include client ID
testuser['name'] = data_type+"_tar_"+str(target) +'_rm_'+str(remain_rate)+'seed_'+str(testuser['seed'])+f'_client{testuser["client_id"]}'
dataset =testuser['name'].split('_tar')[0] 
args_data = get_args(dataset,args)

# Load data for specific client
train_loader, valid_loader, target_loader,testuser['n_class'] = get_acthar(args,data_type ,target , batch_size =64,remain_rate = testuser['remain_data'], seed = testuser['seed'], client_id=testuser['client_id'])
...
# Style conditioner paths - include client-specific paths
conditioner_base = os.getcwd()+os.path.join('/Style_conditioner/conditioner_pth/')
testuser['conditioner'] = conditioner_base + data_type+"_tar_"+str(target) +'_rm_'+str(remain_rate)+'seed_'+str(testuser['seed'])+f'_client{testuser["client_id"]}'+'-'+f'ckp_last-dl.pt'

# Cross-client conditioner paths
testuser['cross_client_conditioners'] = []
if testuser['enable_cross_client_generation']:
    for other_client_id in range(1, testuser['num_clients'] + 1):
        if other_client_id != testuser['client_id']:
            other_conditioner_path = conditioner_base + data_type+"_tar_"+str(target) +'_rm_'+str(remain_rate)+'seed_'+str(testuser['seed'])+f'_client{other_client_id}'+'-'+f'ckp_last-dl.pt'
            testuser['cross_client_conditioners'].append(other_conditioner_path)
...
# Cross-client data path
if testuser['enable_cross_client_generation']:
    testuser['cross_client_newdata'] = os.getcwd()+'/Featurenet/new_data/' +testuser['name']+'-rep'+str(testuser['repeat'])+'-batch'+str(batch_size)+'-cond'+str(testuser['maxcond']) +'-weight'+(str(testuser['cond_weight']))+'-crossclient.pt'

# Use client-specific diffusion model
testuser['diff'] = os.getcwd()+'/Diffusion_model/dm_pth/'+data_type+"_tar_"+str(target) +'_rm_'+str(remain_rate)+'seed_'+str(testuser['seed'])+f'_client{testuser["client_id"]}'+'.pt'
...
if testuser['enable_cross_client_generation']:
    print("Cross-client data:", testuser['cross_client_newdata'])
